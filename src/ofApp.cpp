...
void ofApp::setup(){
  ...
  // cam.setFov(5);
  //cout << "distance : " << cam.getDistance() << '\n';
  //cam.setDistance(100);
  //cout << "distance : " << cam.getDistance() << '\n';
  cout << "key: " << cam.getTranslationKey() << '\n';
  cam.setTranslationKey('k');
  cout << "key: " << cam.getTranslationKey() << '\n';
  loadScene();
  ...
  // audio processing on
	...
	// open patch
	Patch patch = pd.openPatch("pd/empty.pd");
	//Patch patch = pd.openPatch("/home/pi/openFrameworks/apps/myApps/of-meshMappingExample/bin/data/pd/abs");
	//cout << patch << endl;
	
	cout << endl << "close patch" << endl; 
	// close patch
	//pd.closePatch(patch);
	//cout << patch << endl;
	
	cout << endl << "reopen patch" << endl; 
	// open patch again
...
void ofApp::loadScene() {
  ofxLoadCamera(cam, "cameraSettings2");
...
void ofApp::saveScene() {
  ofxSaveCamera(cam, "cameraSettings2");
